...
from mavros_msgs.srv import StreamRate #RCM
...
class gnc_api:
    def __init__(self):
        ...
        rospy.wait_for_service("{}mavros/set_stream_rate".format(self.ns)) #rcm
        
        #rcm:
        self.stream_service_client = rospy.ServiceProxy( 
            name="{}mavros/set_stream_rate".format(self.ns), service_class=StreamRate
        )
    ...
    def start_stream(self):
        ...
        Returns:
                ...
                -1 (int):  unsuccessful.
        """
        response = self.stream_service_client(0,10,1) # here we put the rate and enable=1
        rospy.sleep(1)
        return 0
        #if response.success:
        #    rospy.loginfo(CGREEN2 + "topics streaming successful" + CEND)
        #    return 0
        #else:
        #    rospy.logerr(CRED2 + "topics streaming failed" + CEND)
        #    return -1
    ...
    def check_waypoint_reached(self, pos_tol=1.5, head_tol=0.348):
        ...
        rospy.loginfo(
            CGREEN2 + "x: {}  y: {}  z: {}".format(
                self.current_pose_g.pose.pose.position.x,
                self.current_pose_g.pose.pose.position.y,
                self.current_pose_g.pose.pose.position.z) + CEND)

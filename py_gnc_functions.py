...
class gnc_api:
    def __init__(self):
        ...
        self.global_position_sub = rospy.Subscriber(
            ...
            queue_size=10,
            callback=self.global_position_cb,
        )
    ...
    def enu_2_local(self):
        ...
        # rospy.loginfo(
        #     CYELLOW + "Inside enu_2_local x: {}  y: {}  z: {}".format(
        #         current_pos_local.x,
        #         current_pos_local.y,
        #         current_pos_local.z) + CEND)
    ...
    def set_global_destination(self, lat, lon, alt):
        ...
        self.waypoint_global.pose.position = GeoPoint(lat,lon,alt)

        self.global_pos_pub.publish(self.waypoint_global)

    def edited_set_destination(self, x, y, z, psi):
        """This function is used to command the drone to fly to a waypoint. These waypoints should be specified in the local reference frame. This is typically defined from the location the drone is launched. Psi is counter clockwise rotation following the drone's reference frame defined by the x axis through the right side of the drone with the y axis through the front of the drone.
        ...
        rospy.loginfo(
            "Edited destination set to x:{} y:{} z:{} origin frame".format(x, y, z))
    ...
    def check_global_wp_reached(self, coordinate_tol = 0.3):

        self.global_pos_pub.publish(self.waypoint_global)

        dlat = abs(
            self.waypoint_global.pose.position.latitude - self.global_position.latitude
        )
        dlon = abs(
            self.waypoint_global.pose.position.longitude - self.global_position.longitude
        )

        dMag = sqrt(pow(dlat,2) + pow(dlon,2))

        if dMag < coordinate_tol:
            return 1
        else:
            return 0
    ...
    def edited_check_waypoint_reached(self, x, y, z, psi):
        """This function checks if the waypoint is reached within given tolerance and returns an int of 1 or 0. This function can be used to check when to request the next waypoint in the mission.
        ...
        dx = abs(
            self.waypoint_g.pose.position.x - cur_pose.x
        )
        dy = abs(
            self.waypoint_g.pose.position.y - cur_pose.y
        )
        dz = abs(
            self.waypoint_g.pose.position.z - cur_pose.z
        )

        dMag = sqrt(pow(dx, 2) + pow(dy, 2) + pow(dz, 2))

        rospy.loginfo(
            CVIOLET + "Inside edited_check_waypoint_reached x: {}  y: {}  z: {}".format(
                cur_pose.x,
                cur_pose.y,
                cur_pose.z) + CEND)

        rospy.loginfo(CVIOLET + "The values above should be the same as in enu_2_local" + CEND)
        self.print_coords()
        ...
        if dMag < pos_tol and dHead < head_tol:
            rospy.loginfo(CGREEN + "Magnitude and heading difference is less than the tolerance! CHECKPOINT REACHED" + CEND)
            rospy.loginfo("dMag: {} < pos_tol: {}".format(dMag, pos_tol) + CEND)
            rospy.loginfo("dHead: {} < head_tol: {}".format(dHead, head_tol) + CEND)
            return 1
        else:
            if dMag >= pos_tol:
                rospy.loginfo(CRED + "Magnitude difference is more than tolerance!" + CEND)
                rospy.loginfo("dMag: {} > pos_tol: {}".format(dMag, pos_tol) + CEND)
            if dHead >= head_tol:
                rospy.loginfo(CRED + "Head difference is more than tolerance!" + CEND)
                rospy.loginfo("dHead: {} > head_tol: {}".format(dHead, head_tol) + CEND)
            return 0


    def edited1_check_waypoint_reached(self, x, y, z, psi):
        """This function checks if the waypoint is reached within given tolerance and returns an int of 1 or 0. This function can be used to check when to request the next waypoint in the mission.
        ...
        rospy.loginfo("sqrt({}^2 + {}^2 + {}^2) = {}".format(dx,dy,dz, dMag) + CEND)
        ...
        if dMag < pos_tol and dHead < head_tol:
            ...
            if dMag >= pos_tol:
                ...
                rospy.loginfo(CGREEN + "Magnitude difference is already less than tolerance!" + CEND)
                rospy.loginfo("dMag: {} < pos_tol: {}".format(dMag, pos_tol) + CEND)
            ...
            if dHead >= head_tol:
                ...
                rospy.loginfo(CGREEN + "Head difference is already less than tolerance!" + CEND)
                rospy.loginfo("dHead: {} < head_tol: {}".format(dHead, head_tol) + CEND)
            return 0
    ...
    def print_coords(self):
        ...
        rospy.loginfo(
            CBLUE + "current_pose_g values x: {}  y: {}  z: {} psi: {}".format(
                self.current_pose_g.pose.pose.position.x,
                self.current_pose_g.pose.pose.position.y,
                self.current_pose_g.pose.pose.position.z,
                self.current_heading_g) + CEND)

        cur_pose=Point()
        cur_pose=self.enu_2_local()
        rospy.loginfo(
            CYELLOW + "enu_2_local values x: {}  y: {}  z: {} psi: {}".format(
                cur_pose.x,
                cur_pose.y,
                cur_pose.z,
                self.current_heading_g) + CEND)
    ...
    def print_global_coords(self):
    
        rospy.loginfo(
            CBLUE + "Global values Lat: {}  Long: {}  Alt: {}".format(
                self.global_position.latitude,
                self.global_position.longitude,
                self.global_position.altitude) + CEND)
        return 0
